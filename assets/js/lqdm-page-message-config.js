...
( function (window, document, $, LqdMAdmin) {
    ...
    var $sort_btn = $(".sort-btn"),
        ...
        $reset_btn = $(".reset-btn"),
        $message_message_config_page_wrap = $(".lqdm-message-config-wrap"),
        $message_config_form = $(".lqdm-message-config-form");
        ...
        if (!$(that).hasClass('single')) {
            ...
            if(submitValue === true) {
                ...
                $.ajax({
                    ...
                    method: "POST",
                    data: {
                        action: 'message_message_config_all_series_update',
                    ...
                    $.each(response, function (i, v) {
                        var $form = $message_message_config_page_wrap.find('#lqdm-series-' + i);
                        $.each(v, function (ci, cv) {
                            var $adjcntLabel = $message_message_config_page_wrap.find('label[data-post-id="' + ci + '"]');
     ...
     * after keyup event
     */
    $message_message_config_page_wrap.find('input[type="number"][required]').each(function (i, v) {
        $(v).keyup(function (e) {
    ...
    $message_config_form.on('submit', function (e) {
        ...
        $.ajax({
            ...
            method: "POST",
            data: {
                action: 'message_message_config_single_series_update',
                nonce: LqdMAdmin.ajax_nonce,
            ...
            $.each(response, function (i, v) {
                var $adjcntLabel = $message_message_config_page_wrap.find('label[data-post-id="' + i + '"]');
