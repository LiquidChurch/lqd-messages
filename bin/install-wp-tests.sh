...
# Check if we've been passed sufficient parameters (at least a db-name, db-user)
if [ $# -lt 2 ]; then
...
DB_NAME=$1
DB_USER=$2
# These last three are optional, so include defaults if not provided
DB_PASS=${3-''}
DB_HOST=${4-localhost}
WP_VERSION=${5-latest}
...
echo 'Hey, this is going to create a database or stomp out '"$DB_NAME"' if it exists, continue? [y/n]'
read -r GO_ON
