...
install_required_plugins() {
	...
	if [ ! -d WDS_SHORTCODES ]; then
		...
		# It shouldn't be this way, but to get the tests almost working I had to provide the full path, so replace at to
		# the right of -d with your own path; do the same for CMB2 below.
		unzip /tmp/wds-shortcodes.zip -d /srv/www/wordpress-one/public_html/wp-content/plugins/lqd-messages
	fi

	if [ ! -d CMB2 ]; then
		download https://downloads.wordpress.org/plugin/cmb2.zip /tmp/cmb2.zip
		unzip /tmp/cmb2.zip -d /srv/www/wordpress-one/public_html/wp-content/plugins/lqd-messages
	fi
}
