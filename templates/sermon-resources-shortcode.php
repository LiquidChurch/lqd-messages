<div class="lqdm-resources-wrap <?php $this->output( 'resource_extra_classes', 'esc_attr' ); ?>">
	<ul class="lqdm-resources-list">
        ...
        foreach ($resource_lang as $key => $val) {
            if (empty($items[$val]))
                continue;
            printf('<li class="lqdm-list" style="%s">', $lqdm_list_style);
            printf('<ul class="lqdm-container %s">', $lqdm_container_class);
            printf('<li class="lqdm-head">%s</li>', $lang_plugin_option[$val]);
